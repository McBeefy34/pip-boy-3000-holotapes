...
(function(){
var W=h.getWidth(),H=h.getHeight();
var COLS=20,ROWS=13;
var TW=Math.floor(W/COLS),TH=Math.floor(H/ROWS);
var OX=Math.floor((W-COLS*TW)/2),OY=Math.floor((H-ROWS*TH)/2);
...
var ST_START=0,ST_PLAY=1,ST_PAUSE=2,ST_DEAD=3,ST_CLEAR=4,ST_OVER=5;
var GX=[9,10,9,10],GY=[5,5,6,6],GC=[1,2,3,1];
var MDX=[1,0,-1,0],MDY=[0,1,0,-1];

var map,score,lives,level,gs;
var px,py,pdx,pdy,pendx,pendy;
var ftimer,ptick,gtick,stimer,dots,hiScore;
var tI=null,dI=null,pW=null;
var lk1=0,lk2=0;
...
function mkMap(){
  map=[];
  for(var i=0;i<MAP_SRC.length;i++)map[i]=MAP_SRC.charCodeAt(i)-48;
  dots=0;
...
function drawSprite(c,r){
  ...
  for(var i=0;i<4;i++){
    if(!gde[i]&&!grs[i]&&gx[i]===c&&gy[i]===r){
      if(gsc[i]){
        h.setColor(ftimer<30&&(ptick>>2&1)?3:2).fillRect(cx-3*S,cy-2*S,cx+3*S,cy+2*S);
      } else {
        h.setColor(GC[i]).fillRect(cx-3*S,cy-5*S,cx+3*S,cy+2*S);
        h.setColor(3).fillRect(cx-2*S,cy-3*S,cx-S,cy-2*S).fillRect(cx+S,cy-3*S,cx+2*S,cy-2*S);
...
function spawnG(){
  for(var i=0;i<4;i++){
    gx[i]=GX[i];gy[i]=GY[i];gdx[i]=i%2?-1:1;gdy[i]=0;
    gsc[i]=0;gde[i]=0;grs[i]=0;gmt[i]=0;
  }
}
...
function stepP(){
  ...
  if(px!==opx||py!==opy){markDirty(opx,opy);markDirty(px,py);}
  var c=tile(px,py);
  if(c===0){map[py*COLS+px]=2;score+=10;dots--;}
  else if(c===3){
    map[py*COLS+px]=2;score+=50;dots--;
    ftimer=80+level*5;
    for(var i=0;i<4;i++)if(!gde[i])gsc[i]=1;
  }
  if(dots<=0){gs=ST_CLEAR;stimer=25;}
}
...
function stepG(){
  for(var i=0;i<4;i++){
    ...
    gmt[i]=0;
    var ox=-gdx[i],oy=-gdy[i];
    on=0;
    if(MDX[0]!==ox||MDY[0]!==oy){if(tile(gx[i]+MDX[0],gy[i]+MDY[0])!==1){if(on===0)o0=0;else if(on===1)o1=0;else if(on===2)o2=0;else o3=0;on++;}}
    if(MDX[1]!==ox||MDY[1]!==oy){if(tile(gx[i]+MDX[1],gy[i]+MDY[1])!==1){if(on===0)o0=1;else if(on===1)o1=1;else if(on===2)o2=1;else o3=1;on++;}}
    if(MDX[2]!==ox||MDY[2]!==oy){if(tile(gx[i]+MDX[2],gy[i]+MDY[2])!==1){if(on===0)o0=2;else if(on===1)o1=2;else if(on===2)o2=2;else o3=2;on++;}}
    if(MDX[3]!==ox||MDY[3]!==oy){if(tile(gx[i]+MDX[3],gy[i]+MDY[3])!==1){if(on===0)o0=3;else if(on===1)o1=3;else if(on===2)o2=3;else o3=3;on++;}}
    if(on===0){gdx[i]=ox;gdy[i]=oy;continue;}
    var ci;
    if(gsc[i]||Math.random()<0.3){
      ci=on===1?o0:on===2?(Math.random()<0.5?o0:o1):Math.random()<0.33?o0:Math.random()<0.5?o1:o2;
    } else {
      var bd=999,dd,cx2,co=o0;
      if(on>0){cx2=o0;dd=Math.abs(gx[i]+MDX[cx2]-px)+Math.abs(gy[i]+MDY[cx2]-py);if(dd<bd){bd=dd;co=cx2;}}
      if(on>1){cx2=o1;dd=Math.abs(gx[i]+MDX[cx2]-px)+Math.abs(gy[i]+MDY[cx2]-py);if(dd<bd){bd=dd;co=cx2;}}
      if(on>2){cx2=o2;dd=Math.abs(gx[i]+MDX[cx2]-px)+Math.abs(gy[i]+MDY[cx2]-py);if(dd<bd){bd=dd;co=cx2;}}
      if(on>3){cx2=o3;dd=Math.abs(gx[i]+MDX[cx2]-px)+Math.abs(gy[i]+MDY[cx2]-py);if(dd<bd){bd=dd;co=cx2;}}
      ci=co;
    }
    markDirty(gx[i],gy[i]);
    gdx[i]=MDX[ci];gdy[i]=MDY[ci];gx[i]+=MDX[ci];gy[i]+=MDY[ci];
    markDirty(gx[i],gy[i]);
  }
}
...
function onTick(){
  if(gs===ST_DEAD||gs===ST_CLEAR){
    stimer--;
    if(stimer<=0){
      if(gs===ST_DEAD){spawnP();spawnG();ftimer=0;gs=ST_PLAY;h.clear(1);drawFullMaze();drawHUD();dirX=[];dirY=[];}
      else{level++;score+=500*level;mkMap();spawnP();spawnG();ftimer=0;gs=ST_PLAY;drawFullMaze();}
    }
    return;
  }
  if(gs!==ST_PLAY)return;
  gtick=(gtick+1)&255;ptick=(ptick+1)&255;
  if(ftimer>0){ftimer--;if(ftimer===0)for(var i=0;i<4;i++)if(!gde[i])gsc[i]=0;}
  stepP();stepG();chkCol();
}
...
function onDraw(){
  if(gs===ST_START){
    h.clear(1);
    h.setFontMonofonto28().setFontAlign(0,0).setColor(3).drawString('PIP MAN',W/2,H/2-60);
    h.setFont('6x8',2).setColor(2).drawString('Click to START',W/2,H/2-20);
    if(hiScore>0){
      h.setFont('6x8',2).setColor(2).setFontAlign(0,0).drawString('HIGH SCORE',W/2,H/2+5);
      h.setFont('6x8',4).setColor(1).setFontAlign(0,0).drawString(''+hiScore,W/2,H/2+30);
    }
    h.setFont('6x8',2).setColor(3).setFontAlign(0,0)
...
function reset(){
  mkMap();score=0;lives=3;level=1;ftimer=0;ptick=0;gtick=0;
  gs=ST_PLAY;spawnP();spawnG();
  h.clear(1);drawFullMaze();drawHUD();h.flip();Pip.lastFlip=getTime();
  dirX=[];dirY=[];
}
